...
def patch_angle_calc(data:dict):
    ...
    for t_stamp, ns in ns_series.time_capsule.items():
        ...
        for idx_1, ia1 in enumerate(arms_idx):
            ...
            for idx_2 in range(idx_1+1, len(arms_idx)):
                ...
                is_sharing_strand = True if len(set([arm1.strand_id_0,arm1.strand_id_1,arm2.strand_id_0,arm2.strand_id_1])) == 3 else False
    ...
    p_angs_vtime_dic.params['Arm_Pairs'] = [(ia1, arms_idx[idx_2]) for idx_1, ia1 in enumerate(arms_idx) for idx_2 in range(idx_1+1, len(arms_idx))]
