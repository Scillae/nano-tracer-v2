    ...
    for t_stamp, ns in ns_series.items():
        ...
        if mismatch_b_ls: # empty is False; enter here if not empty
            ...
            for bp in mismatch_bp_ls:
                ...
                fbp_dist_ls = [CoM_bp(fbp)-CoM_bp(bp) for fbp in fbp_dic.values()] # find the distance between the 'true' start and the re-matched bp
                match_cand_ls = sorted(zip(fbp_dist_ls, fbp_dic.values()),key=lambda x: np.linalg.norm(x[0])) # sort the candidate re-matched bps according to distance
        ...
        for ia, fbp_idx in shift_dic.items():
            if type(fbp_idx) is int:
                if fbp_idx == 1: # intact arm; no modification required
                    continue
                arm = ns.arms[ia]
                arm.base_pairs = generate_new_arm_base_pairs(arm,fbp_idx) # generate a new arm that starts from the 'true' start
                ns.arms[ia] = arm
            else: # fbp_idx == {dist: bp}
                arm = ns.arms[ia]
                arm.base_pairs = generate_new_arm_base_pairs(arm,-1*len(fbp_idx), bp_append = [d_bp[1] for d_bp in sorted(list(fbp_idx.items()), key=lambda x:x[0], reverse=True)]) # generate a new arm that starts from the re-matched bps; the farthest re-matched bp should be the 1st bp of the arm
                ns.arms[ia] = arm
